...
def edit_distance(s, t):
    dist_1 = textdistance.damerau_levenshtein.normalized_distance(str(s), str(t))
    return dist_1
...
def wpd(s, t):
    ...
    for i in list(s_token_data.keys()):
        ...
        if i in list(t_token_data.keys()):
            if len(s_token_data[i]) == 1 and len(t_token_data[i]) == 1:
                ...
                delta_W = max(combined_position_list) - \
                    min(combined_position_list)
    ...
    if len(delta_W_list) > 0:
        ...
        print("Warning: len(delta_W_list)==0 ; since there is no match in structure found, score=1.0")
        print("s:", s)
        print("t:", t)
        sigma_W = 1.0
    return sigma_W
...
def ld(s, t):
    ...
    if len(total_vocab) > 0:
        ...
        print(
            "Warning: overlap_vocab==0 ; since there is no match in vocab found, score=1.0")
        print("s:", s)
        print("t:", t)
        return 1.0
